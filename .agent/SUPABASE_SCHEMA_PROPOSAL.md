...
- `unique (plant_id, device_id)` so readings can enforce that a reading's `plant_id` matches the device assignment.
...
| `device_id` | `text not null` | Matches incoming payload; enforced with `plant_id` as a composite FK to `devices`. |
...
- `foreign key (plant_id, device_id) references public.devices(plant_id, device_id)` to reject readings attributed to the wrong plant/device pair.
...
Implementation note: the ingestion path should derive `plant_id` from `devices.device_id` rather than trusting a client-provided plant id. If devices can move between plants later, add a `device_plant_assignments` table with effective timestamps and have readings reference that assignment instead of only the current `devices` row.
